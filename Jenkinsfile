pipeline {
    agent any
    environment {
        DOCKER_IMAGE = "jeeva31/devops-sample:latest"  // Change this to your registry
    ...
    stages {
        stage('Checkout Code') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'github-jeeva', usernameVariable: 'GIT_USER', passwordVariable: 'GIT_TOKEN')]) {
                    git url: "https://$GIT_USER:$GIT_TOKEN@github.com/Jeeva-21BSR017/devops-sample.git", branch: 'main'
                }
            }
        }
        ...
        stage('Login to Docker Registry') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'github-jeeva31', usernameVariable: 'DOCKER_USER', passwordVariable: 'DOCKER_PASS')]) {
